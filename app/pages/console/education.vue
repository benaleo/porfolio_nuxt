  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500">Create and manage your education history</p>
      </div>
      <button @click="startCreate" class="rounded bg-blue-600 text-white px-4 py-2 hover:bg-blue-700">New Education</button>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_420px]">
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-if="(education || []).length === 0">
              <td colspan="5" class="px-4 py-6 text-center text-slate-500">No education records yet</td>
            </tr>
          </tbody>
        </table>
        <!-- Pagination -->
      ...
      <form v-if="editing" class="rounded-xl border border-slate-200 dark:border-slate-800 p-4 bg-white/80 dark:bg-slate-900/80 space-y-3" @submit.prevent="onSave">
        <div class="text-sm font-medium">{{ form.id ? 'Edit Education' : 'New Education' }}</div>
        <div>
          <label class="block text-sm mb-1">Year</label>
          <input v-model="form.year" type="text" class="w-full rounded border border-slate-300 dark:border-slate-700 bg-transparent px-3 py-2 outline-none focus:ring focus:ring-blue-200" required />
        </div>
        <div>
          <label class="block text-sm mb-1">Institution</label>
          <input v-model="form.institution" type="text" class="w-full rounded border border-slate-300 dark:border-slate-700 bg-transparent px-3 py-2 outline-none" required />
        </div>
        <div>
          <label class="block text-sm mb-1">Major</label>
          <input v-model="form.major" type="text" class="w-full rounded border border-slate-300 dark:border-slate-700 bg-transparent px-3 py-2 outline-none" required />
        </div>
        <div>
...
const { data: education, refresh } = await useAsyncData('education-list', () => $fetch<Education[]>('/api/education'))

const page = ref(1)
const pageSize = ref(10)
const totalPages = computed(() => {
  const total = (education.value || []).length
  return Math.max(1, Math.ceil(total / pageSize.value))
})
const paginated = computed(() => {
  const list = education.value || []
  const start = (page.value - 1) * pageSize.value
  return list.slice(start, start + pageSize.value)
})
watch([education, pageSize], () => { page.value = 1 })
...
const form = reactive<{ id?: number; year: string; institution: string; major: string; summary?: string | null }>(
  { year: '', institution: '', major: '', summary: '' },
)

const startCreate = () => {
  editing.value = true
  Object.assign(form, { id: undefined, year: '', institution: '', major: '', summary: '' })
}

const startEdit = (e: Education) => {
  editing.value = true
  Object.assign(form, { id: e.id, year: e.year, institution: e.institution, major: e.major, summary: e.summary || '' })
}
...
const onSave = async () => {
  ...
  try {
    if (!form.id) {
      await $fetch('/api/education', { method: 'POST', credentials: 'include', body: form })
    } else {
      await $fetch(`/api/education/${form.id}`, { method: 'PATCH', credentials: 'include', body: form })
    }
    await refresh()
    editing.value = false
  } catch (e: any) {
...
const onDelete = async (e: Education) => {
  ...
  try {
    await $fetch(`/api/education/${e.id}`, { method: 'DELETE', credentials: 'include' })
    await refresh()
  } catch (err: any) {
