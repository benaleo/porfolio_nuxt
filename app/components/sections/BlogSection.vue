<template>
  <section class="py-20" id="blog">
    <div class="mx-auto max-w-6xl px-4">
      ...
      <div v-if="blogs.length" class="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <NuxtLink
          v-for="b in blogs"
          :key="b.slug"
          :to="`/blog/${b.slug}`"
          ...
          :enter="{ opacity: 1, y: 0 }"
        >
          <div class="text-xs text-slate-500">{{ format(b.date) }}</div>
          <div class="mt-1 font-semibold">{{ b.title }}</div>
          <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">{{ b.snippet }}</p>
        </NuxtLink>
      </div>
